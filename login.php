...
if (isset($_POST[login])){
 ...
 $admin = mysql_query("SELECT * FROM users WHERE username='".anti_injection($_POST[a])."' AND password='$pass'");
 $guru = mysql_query("SELECT * FROM guru WHERE nip='".anti_injection($_POST[a])."' AND password='$passlain'");
 $siswa = mysql_query("SELECT * FROM siswa WHERE nisn='".anti_injection($_POST[a])."' AND password='$passlain'");
 ...
 if ($hitungadmin >= 1){
    ...
    mysql_query("INSERT INTO users_aktivitas VALUES('','$r[id_user]','$ip','$user_browser $version','$user_os','$r[level]','".date('H:i:s')."','".date('Y-m-d')."')");
    ...
    $_SESSION[level]    = 'guru';
    include "config/user_agent.php";
    mysql_query("INSERT INTO users_aktivitas VALUES('','$r[nip]','$ip','$user_browser $version','$user_os','guru','".date('H:i:s')."','".date('Y-m-d')."')");
    echo "<script>document.location='index.php';</script>";
    ...
    include "config/user_agent.php";
    mysql_query("INSERT INTO users_aktivitas VALUES('','$r[nisn]','$ip','$user_browser $version','$user_os','siswa','".date('H:i:s')."','".date('Y-m-d')."')");
    echo "<script>document.location='index.php';</script>";
 }else{
