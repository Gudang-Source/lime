<?php
$cek = mysql_num_rows(mysql_query("SELECT * FROM psb_aktivasi where kode_pendaftaran='$_GET[kode]'"));
$d = mysql_fetch_array(mysql_query("SELECT * FROM psb_aktivasi where kode_pendaftaran='$_GET[kode]'"));
if ($cek <= 0){
	echo "<script>document.location='index.mu';</script>";
}else{
	if (isset($_POST['simpan'])){
		$cek = mysql_num_rows(mysql_query("SELECT * FROM psb_pendaftaran where kode_pendaftaran='$_GET[kode]'"));
		if ($cek >= 1){
			echo "<script>document.location='index.mu';</script>";
		}else{
			...
			$status = $d[status];
			mysql_query("UPDATE psb_aktivasi SET proses='1' where kode_pendaftaran='$kode'");
			mysql_query("INSERT INTO psb_pendaftaran VALUES('','$kode','$a','$b','$c','$d','$e','$tgllahir','$f','$g','$h','$i','$j','$k','$l','$m','$n','$o','$p',
			...
			$idd = mysql_insert_id();
			   for ($i=0; $i<=6; $i++){
			     if (isset($_POST['sa'.$i])){
			        ...
			        if (trim($nama) != ''){
			       		mysql_query("INSERT INTO psb_pendaftaran_saudara VALUES('','$idd','$nama','$umur','$pendidikan')");
			       	}
			     }
			   }
