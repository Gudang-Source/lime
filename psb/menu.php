              ...
              <?php
                $menu = mysql_query("SELECT * FROM menu where id_parent='0' AND aktif='Ya' AND status='psb' ORDER BY urutan ASC");
                while($dataMenu = mysql_fetch_assoc($menu)){
                  $menu_id = $dataMenu['id_menu'];
                  $submenu = mysql_query("SELECT * FROM menu WHERE id_parent='$menu_id' AND aktif='Ya' AND status='psb' ORDER BY urutan ASC");
