<?php
	if (isset($_GET[cek])){
		...
		$cek = mysql_fetch_array(mysql_query("SELECT * FROM psb_aktivasi where kode_pendaftaran='$kode'"));
		$total = mysql_num_rows(mysql_query("SELECT * FROM psb_aktivasi where kode_pendaftaran='$kode'"));
		if ($cek[proses] == 1){
...
$query = mysql_query("SELECT * FROM halaman where id_halaman='1' AND status='psb'");
