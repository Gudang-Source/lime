...
echo "<table id='tablemodul1' width=100% border=1>
      ...
      echo "<tr>
        ...
        while ($m = mysql_fetch_array($mapel)){
          $n = mysql_fetch_array(mysql_query("SELECT * FROM nilai_uts where kodejdwl='$m[kodejdwl]' AND nisn='$s[nisn]'"));
          $cekpredikat = mysql_num_rows(mysql_query("SELECT * FROM predikat where kode_kelas='$_GET[kelas]'"));
                      if ($cekpredikat >= 1){
                        $grade1 = mysql_fetch_array(mysql_query("SELECT * FROM `predikat` where ($n[angka_pengetahuan] >=nilai_a) AND ($n[angka_pengetahuan] <= nilai_b) AND kode_kelas='$_GET[kelas]'"));
                        $grade2 = mysql_fetch_array(mysql_query("SELECT * FROM `predikat` where ($n[angka_keterampilan] >=nilai_a) AND ($n[angka_keterampilan] <= nilai_b) AND kode_kelas='$_GET[kelas]'"));
                      }else{
                        $grade1 = mysql_fetch_array(mysql_query("SELECT * FROM `predikat` where ($n[angka_pengetahuan] >=nilai_a) AND ($n[angka_pengetahuan] <= nilai_b) AND kode_kelas='0'"));
                        $grade2 = mysql_fetch_array(mysql_query("SELECT * FROM `predikat` where ($n[angka_keterampilan] >=nilai_a) AND ($n[angka_keterampilan] <= nilai_b) AND kode_kelas='0'"));
                      }
        echo "<tr>
