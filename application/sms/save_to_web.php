...
if ($code == '19891989'){
	if ($op == 'send'){
		...
		$qr = mysql_query("SELECT * FROM sms_autoreply where keyword='$pesan'");
		$ar = mysql_num_rows($qr);
		$r = mysql_fetch_array($qr);
		if ($ar >= 1){
			mysql_query("INSERT INTO sms VALUES('','$notelp','$r[isi_pesan]')");
			mysql_query("INSERT INTO sms_outbox VALUES('','$notelp','$r[isi_pesan]','".date('Y-m-d H:i:s')."')");
		}
	}
}
echo "</data>";
