              ...
              <input type='hidden' name='jdwl' value='$_GET[jdwl]'>
                <div class='col-md-12'>
                  <table class='table table-condensed table-bordered table-striped'>
                    ...
                    $no = 1;
                    $tampil = mysql_query("SELECT * FROM siswa a JOIN jenis_kelamin b ON a.id_jenis_kelamin=b.id_jenis_kelamin where a.kode_kelas='$_GET[id]' ORDER BY a.id_siswa");
                    while($r=mysql_fetch_array($tampil)){
                    if ($_GET[gettgl]){
                    ...
                    $a = mysql_fetch_array(mysql_query("SELECT * FROM absensi_siswa where kodejdwl='$_GET[jdwl]' AND tanggal='$sekarangabsen' AND nisn='$r[nisn]'"));
                    echo "<tr bgcolor=$warna>
                            ...
                            <td><select style='width:100px;' name='a[$no]' class='form-control'>";
                                  $kehadiran = mysql_query("SELECT * FROM kehadiran");
              ...
              echo "</form>
                ...
                if (isset($_POST[simpann])){
                    ...
                    for ($i=1; $i <= $jml_data; $i++){
                      $cek = mysql_query("SELECT * FROM absensi_siswa where kodejdwl='$_POST[jdwl]' AND nisn='".$nisn[$i]."' AND tanggal='".$e."-".$f."-".$g."'");
                      $total = mysql_num_rows($cek);
                      if ($total >= 1){
                        mysql_query("UPDATE absensi_siswa SET kode_kehadiran = '".$a[$i]."' where nisn='".$nisn[$i]."' AND kodejdwl='$_POST[jdwl]'");
                        $cs = mysql_fetch_array(mysql_query("SELECT * FROM siswa a JOIN kelas b ON a.kode_kelas=b.kode_kelas where a.nisn='".$nisn[$i]."'"));
                        if ($a[$i] != 'H'){
                          ...
                          if ($cs[no_telpon_ayah] != ''){
                              mysql_query("INSERT INTO sms VALUES('','$cs[no_telpon_ayah]','$isi_pesan')");
                          }elseif ($cs[no_telpon_ibu] != ''){
                              mysql_query("INSERT INTO sms VALUES('','$cs[no_telpon_ibu]','$isi_pesan')");
                          }
                        }
                      }else{
                        mysql_query("INSERT INTO absensi_siswa VALUES('','$_POST[jdwl]','".$nisn[$i]."','".$a[$i]."','".$e."-".$f."-".$g."','".date('Y-m-d H:i:s')."')");
                        $cs = mysql_fetch_array(mysql_query("SELECT * FROM siswa a JOIN kelas b ON a.kode_kelas=b.kode_kelas where a.nisn='".$nisn[$i]."'"));
                        if ($a[$i] != 'H'){
                          if ($a[$i] == 'A'){ $statush = 'Alpa'; }elseif ($a[$i] == 'S'){ $statush = 'Sakit'; }elseif ($a[$i] == 'I'){ $statush = 'Izin'; }
                          $isi_pesan = "Diberitahukan kepada Yth Bpk/Ibk, Bahwa anak anda $cs[nama], $cs[nama_kelas] absensi Hari ini Tanggal $g-$f-$e : $statush";
    ...
    echo "<div class='col-xs-12'>  
              <div class='box'>
                ...
                <table class='table table-bordered table-striped'>
                    ...
                    $tampil = mysql_query("SELECT * FROM jadwal_pelajaran a 
