              ...
              <div class="box">
                ...
                <div class="box-body">
                  ...
                  <?php
                    ...
                    $a = mysql_fetch_array(mysql_query("SELECT * FROM absensi_guru where kodejdwl='$r[kodejdwl]' AND nip='$r[nip]' AND tanggal='$sekarangabsen'"));
                    echo "<tr><td>$no</td>
...
<?php 
    if (isset($_POST[simpan])){
      ...
      for ($i=1; $i <= $jml_data; $i++){
        $cek = mysql_query("SELECT * FROM absensi_guru where kodejdwl='".$jdwl[$i]."' AND nip='".$nip[$i]."' AND tanggal='$tglabsen'");
        $total = mysql_num_rows($cek);
        if ($total >= 1){
           mysql_query("UPDATE absensi_guru SET kode_kehadiran = '".$a[$i]."' where nip='".$nip[$i]."' AND kodejdwl='".$jdwl[$i]."' AND tanggal='$tglabsen'");
        }else{
             mysql_query("INSERT INTO absensi_guru VALUES ('',
                                                              '".$jdwl[$i]."',
                                                              '".$nip[$i]."',
                                                              '".$a[$i]."',
                                                              '$tglabsen',
