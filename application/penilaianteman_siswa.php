              ...
              <form action='' method='POST'>
                ...
                <div class="box-body">
                  <table class="table table-bordered table-striped">
                    ...
                    <tbody>
                  <?php 
                    $cs = mysql_fetch_array(mysql_query("SELECT * FROM siswa where nisn='$_SESSION[id]'"));
...
<?php 
    ...
    if (isset($_POST['submit'])){
       $jml = mysql_fetch_array(mysql_query("SELECT count(*) as jmlp FROM `pertanyaan_penilaian` where status='teman'"));
       $n = $jml[jmlp];
       for ($i=1; $i<=$n; $i++){
         if (isset($_POST['jawab'.$i])){
           ...
           $nisn = $_POST['nisn'.$i];
           $kelas = $_POST['kelas'.$i];
            $cek = mysql_fetch_array(mysql_query("SELECT count(*) as tot FROM pertanyaan_penilaian_jawab where nisn='$_SESSION[id]' AND id_pertanyaan_penilaian='$pertanyaan' AND nisn_teman='$nisn' AND status='teman' AND kode_kelas='$kelas'"));
            if ($cek[tot] >= 1){
              mysql_query("UPDATE pertanyaan_penilaian_jawab SET jawaban='$jawab' where id_pertanyaan_penilaian='$pertanyaan' AND nisn='$_SESSION[id]' AND kode_kelas='$kelas'");
            }else{
              mysql_query("INSERT INTO pertanyaan_penilaian_jawab VALUES('','$pertanyaan','$_SESSION[id]','$nisn','$jawab','teman','$kelas','".date('Y-m-d H:i:s')."')");
          }
         }
       }
              ...
              <form action='' method='POST'>
                ...
                <div class="box-body">
                  <?php
                      ...
                      $tt = mysql_fetch_array(mysql_query("SELECT * FROM siswa where nisn='$_SESSION[id]'"));
                  ...
                  <?php 
                    ...
                    $no = 1;
                    while($r=mysql_fetch_array($tampil)){
                    $jwb = mysql_fetch_array(mysql_query("SELECT * FROM pertanyaan_penilaian_jawab where nisn='$_SESSION[id]' AND nisn_teman='$_GET[nisn]' AND id_pertanyaan_penilaian='$r[id_pertanyaan_penilaian]' AND status='teman' AND kode_kelas='$tt[kode_kelas]'"));
                    echo "<tr><td>$no</td>
                              <td>$r[pertanyaan]</td>
                          </tr>
                          <tr><td></td>
