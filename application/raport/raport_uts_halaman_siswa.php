<?php 
  echo "<div class='col-xs-12'>  
      ...
      echo "<tr>
        ...
        $n = mysql_fetch_array(mysql_query("SELECT * FROM nilai_uts where kodejdwl='$m[kodejdwl]' AND nisn='$iden[nisn]'"));
        $cekpredikat = mysql_num_rows(mysql_query("SELECT * FROM predikat where kode_kelas='$_SESSION[kode_kelas]'"));
        if ($cekpredikat >= 1){
          $grade1 = mysql_fetch_array(mysql_query("SELECT * FROM `predikat` where ($n[angka_pengetahuan] >=nilai_a) AND ($n[angka_pengetahuan] <= nilai_b) AND kode_kelas='$_SESSION[kode_kelas]'"));
          $grade2 = mysql_fetch_array(mysql_query("SELECT * FROM `predikat` where ($n[angka_keterampilan] >=nilai_a) AND ($n[angka_keterampilan] <= nilai_b) AND kode_kelas='$_SESSION[kode_kelas]'"));
        }else{
